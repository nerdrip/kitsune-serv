...
try {
    $selfCheck = pm_ApiCli::callSbin('kitsuneserv-bridge-r15', ['--self-check'], pm_ApiCli::RESULT_FULL);
} catch (Throwable $exception) {
    throw new RuntimeException('Nie udało się uruchomić uprzywilejowanego executora KitsuneServ Bridge r15 przez Plesk.', 0, $exception);
}
if ((int) ($selfCheck['code'] ?? 1) !== 0 || trim((string) ($selfCheck['stdout'] ?? '')) !== '3.1.1-r15') {
    throw new RuntimeException('Executor KitsuneServ Bridge r15 nie przeszedł kontroli wersji przez Plesk.');
}
...
if (($config['deployment_mode'] ?? 'managed') === 'managed' && ($config['proxy_mode'] ?? 'manual') === 'managed' && Modules_KitsuneservBridge_Config::proxyDomains($config)) {
    ...
    try {
        ...
        $proxyResult = pm_ApiCli::callSbin('kitsuneserv-bridge-r15', ['--config', $runtimeConfig], pm_ApiCli::RESULT_FULL);
