...
- Synchronize project, Test Lab and API Flow definitions through an explicit Git-style compare/pull/push/merge plan: one-sided changes keep a clear direction, independent field edits use a three-way merge, overlapping edits remain manual conflicts, and every forced replacement keeps revision history; secret values and project-directory files never enter synchronized manifests
...
- Install `artifacts/plesk/kitsuneserv-bridge-3.1.3-r11.zip` in Plesk, select the Hub domain and a direct base API subdomain such as `api.panel.example.com`. Bridge treats it as a namespace: starting an API Flow named `Orders` publishes `orders.api.panel.example.com`, adds wildcard nginx routing and attempts to create the wildcard DNS CNAME. A wildcard TLS certificate for the base API domain is still required. Plesk Management checks every managed plugin's configured repository and `main` branch directly, fast-forwards its protected checkout when needed, and installs only an archive built from the pinned commit. SSH host keys are discovered on first use, persisted privately and then enforced with strict host verification. Managed synchronizations detect and schedule newer Bridge releases from the same verified checkout automatically; the Deployment tab explicitly fetches the configured branch and shows installed versus repository releases before enabling an upgrade. The Showcase tab publishes each library's `main` documentation as stable and `develop` documentation as snapshot on a selected Plesk domain. Automatic signed SSO enrollment does not require copying a pairing code.
