...
test('Showcase publishes unified stable and snapshot documentation from configured repositories', () => {
  ...
  assert.doesNotMatch(site, /class="arrow"/);
  assert.match(site, /Cache-Control: no-store, no-cache, must-revalidate/);
  assert.match(site, /kitsune-showcase-template/);
  assert.match(manager, /template-release=/);
  assert.match(manager, /template-sha256=/);
