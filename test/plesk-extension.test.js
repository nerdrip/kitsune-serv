...
class pm_ApiCli { const RESULT_FULL = 1; public static function callSbin($command, $arguments, $result) { if ($command !== 'kitsuneserv-bridge-r1' || $arguments !== ['--self-check']) throw new RuntimeException('Unexpected privileged call'); return ['code' => 0, 'stdout' => "3.1.3-r9\\n", 'stderr' => '']; } }
...
`;
  ...
  const canonicalPayload = { connectorId: 'plesk-test', timestamp: 1800000000000, nonce: '0123456789abcdef0123456789abcdef', device: { capabilities: ['inventory', 'plesk-sso'], name: 'Plesk Łódź', platform: 'Linux', version: '3.1.3-r9' } };
...
test('managed deployment discovers and propagates a compatible Plesk Node.js runtime', () => {
  ...
  assert.match(manager, /KITSUNESERV_BRIDGE_EXECUTOR_RELEASE = '3\.1\.3-r9'/);
