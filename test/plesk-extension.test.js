...
test('Kitsune Hub centralizes suite navigation and validates uploaded extension packages', () => {
  ...
  assert.match(controller, /suiteRefreshAction/);
  assert.match(controller, /suiteUpdateAction/);
  for (const marker of ['REPOSITORIES', 'refreshCatalog', 'updateFromCatalog', 'repositoryOperation', "callSbin('kitsune-suite-self-update'"]) assert.ok(suite.includes(marker), 'missing repository update marker ' + marker);
  assert.match(view, /Kitsune Plesk Management/);
  assert.match(view, /Sprawdź aktualizacje/);
  assert.match(view, /Zainstalowano/);
  assert.match(view, /Repozytorium/);
...
test('suite template and aggregate update builder preserve one navigation contract', () => {
  ...
  assert.match(repositoryRunner, /effectiveRepositoryUrl/);
  assert.match(repositoryRunner, /git@github\.com:/);
  assert.match(repositoryRunner, /Prywatne repozytorium GitHub wymaga tokenu HTTPS albo globalnego klucza SSH/);
...
test('managed deployment protects credentials, data paths, service and Plesk-compatible proxy changes', () => {
  ...
  assert.match(manager, /chmod\(\$trustFile, 0600\)/);
...
test('Showcase publishes unified stable and snapshot documentation from configured repositories', () => {
  const controller = fs.readFileSync(path.join(extension, 'plib/controllers/IndexController.php'), 'utf8');
  const config = fs.readFileSync(path.join(extension, 'plib/library/Config.php'), 'utf8');
  const manager = fs.readFileSync(path.join(extension, 'sbin/kitsuneserv-bridge-r1'), 'utf8');
  const view = fs.readFileSync(path.join(extension, 'plib/views/scripts/index/index.phtml'), 'utf8');
  ...
  assert.match(config, /getDocumentRoot/);
  assert.match(config, /getHomePath/);
  ...
  assert.match(manager, /\['main' => 'stable', 'develop' => 'snapshot'\]/);
  assert.match(manager, /docs\/index\.html/);
  assert.match(manager, /StrictHostKeyChecking=yes/);
  assert.match(manager, /ssh-keyscan/);
  assert.match(manager, /git-known-hosts/);
  assert.match(manager, /repositorySshEndpoint/);
  assert.doesNotMatch(manager, /SSH repository requires verified known_hosts content/);
  assert.match(manager, /showcase-backup/);
  assert.match(manager, /\.kitsune-showcase/);
  assert.match(manager, /showcase_home_path/);
  assert.match(site, /showcase\.json/);
  assert.match(site, /stable_url/);
  assert.match(site, /snapshot_url/);
  assert.match(site, /stable_version/);
  assert.match(site, /snapshot_version/);
  assert.match(site, /class="channel-link/);
  assert.match(site, /class="card-head"/);
  assert.doesNotMatch(site, /\$c\['url'\]/);
  assert.match(site, /class="special-tools"/);
  assert.match(site, /layout-editor/);
  assert.match(site, /playground/);
  assert.match(manager, /\$version \.= '-SNAPSHOT'/);
...
test('all extension PHP sources and the privileged post-install self-check pass when PHP is available', { skip: spawnSync('php', ['-v'], { stdio: 'ignore' }).status !== 0 }, t => {
  ...
  phpSources.push(path.join(extension, 'sbin/kitsuneserv-bridge-r1'));
...
class pm_ApiCli { const RESULT_FULL = 1; public static function callSbin($command, $arguments, $result) { if ($command !== 'kitsuneserv-bridge-r1' || $arguments !== ['--self-check']) throw new RuntimeException('Unexpected privileged call'); return ['code' => 0, 'stdout' => "3.1.3-r10\\n", 'stderr' => '']; } }
...
`;
  ...
  const canonicalPayload = { connectorId: 'plesk-test', timestamp: 1800000000000, nonce: '0123456789abcdef0123456789abcdef', device: { capabilities: ['inventory', 'plesk-sso'], name: 'Plesk Łódź', platform: 'Linux', version: '3.1.3-r10' } };
...
test('managed deployment discovers and propagates a compatible Plesk Node.js runtime', () => {
  ...
  assert.match(installer, /callSbin\('kitsuneserv-bridge-r1', \['--self-check'\]/);
  ...
  assert.match(manager, /KITSUNESERV_BRIDGE_EXECUTOR_RELEASE = '3\.1\.3-r10'/);
  ...
  assert.match(operations, /callSbin\('kitsuneserv-bridge-r1'/);
