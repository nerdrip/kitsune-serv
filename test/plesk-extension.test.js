...
test('Plesk extension has an installable SDK structure and release metadata', () => {
  const required = [
    ...
    'plib/hooks/LongTasks.php', 'plib/hooks/WebServer.php', 'plib/scripts/post-install.php', 'plib/scripts/pre-uninstall.php', 'sbin/kitsuneserv-bridge-r16'
  ];
  ...
  assert.match(meta, /<release>16<\/release>/);
...
test('managed deployment protects credentials, data paths, service and Plesk-compatible proxy changes', () => {
  const manager = fs.readFileSync(path.join(extension, 'sbin/kitsuneserv-bridge-r16'), 'utf8');
  const webServer = fs.readFileSync(path.join(extension, 'plib/hooks/WebServer.php'), 'utf8');
...
test('all extension PHP sources and the privileged post-install self-check pass when PHP is available', { skip: spawnSync('php', ['-v'], { stdio: 'ignore' }).status !== 0 }, t => {
  ...
  phpSources.push(path.join(extension, 'sbin/kitsuneserv-bridge-r16'));
...
class pm_ApiCli { const RESULT_FULL = 1; public static function callSbin($command, $arguments, $result) { if ($command !== 'kitsuneserv-bridge-r16' || $arguments !== ['--self-check']) throw new RuntimeException('Unexpected privileged call'); return ['code' => 0, 'stdout' => "3.1.1-r16\\n", 'stderr' => '']; } }
...
`;
  ...
  const canonicalPayload = { connectorId: 'plesk-test', timestamp: 1800000000000, nonce: '0123456789abcdef0123456789abcdef', device: { capabilities: ['inventory', 'plesk-sso'], name: 'Plesk Łódź', platform: 'Linux', version: '3.1.1-r16' } };
...
test('managed deployment discovers and propagates a compatible Plesk Node.js runtime', () => {
  ...
  assert.match(installer, /callSbin\('kitsuneserv-bridge-r16', \['--self-check'\]/);
  ...
  assert.match(manager, /KITSUNESERV_BRIDGE_EXECUTOR_RELEASE = '3\.1\.1-r16'/);
  ...
  assert.match(operations, /callSbin\('kitsuneserv-bridge-r16'/);
  assert.doesNotMatch(operations, /kitsuneserv-bridge-r(?:9|10|11|12|13|14|15)/);
